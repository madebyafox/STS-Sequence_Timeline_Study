				...
				if (empty($seqs) == 0) 
					{
		  			...
		  			if(count($seqs) != 28) echo "ERROR IN ROW COUNT";
						else 
						{
							...
							if ($decision == "D")
							{
								$score = calcReasoningForDefendent($score,$dir,$seqs);
							}
							else if ($decision == "P")
							{
								$score = calcReasoningForPlaintiff($score,$dir,$seqs);
							}
							else echo "ERROR: NO DECISION";
...
function calcReasoningForDefendent($score,$dir,$seqs)
{
	global $plaintiff;
	global $cSubject;
	
	$score = $score + rule0($seqs,$dir);
	$score = $score + rule1($seqs,$dir);
	$score = $score + rule2($seqs,$dir);
	$score = $score + rule3($seqs,$dir);
	$score = $score + rule4($seqs,$dir);
	$score = $score + rule5($seqs,$dir);
	$score = $score + rule6($seqs,$dir);
	$score = $score + 3*rule7($seqs,$dir);
	$score = $score + 3*rule8($seqs,$dir);
	$score = $score + rule9($seqs,$dir);
	
	$score = $score + rule11M($seqs,$dir);
	$score = $score + rule11C($seqs,$dir);
	...
	switch ($light)
	{
		case "GREEN":
			$score = $score + 3;
			writeScore($cSubject,"MLight",3,$dir,"FOR D: M had green");
	...
	switch ($signal)
	{
		...
		case "RED":
			$score = $score + 3;
			writeScore($cSubject,"CSignal",3,$dir,"FOR M: C on DON'TWALK");
	...
	{
		$score = $score + 2;
		writeScore($cSubject,"consist",2,$dir,"");
	}
	

	//--FIND FOR MOTORIST RULES --------
  // $score = $score + rule10M($seqs,$dir);
 	// $score = $score + rule12M($seqs,$dir);
 	//--FIND FOR CYCLIST RULES --------
	// 	$score = $score - rule10C($seqs,$dir);
	// 	$score = $score + rule12C($seqs,$dir);
	
	return $score;
}

function calcReasoningForPlaintiff($score,$dir,$seqs)
{
	global $plaintiff;
	global $cSubject;
  ...
  $score = $score + rule11M($seqs,$dir);
	...
	switch ($light)
	{
		...
		case "RED":
			$score = $score + 3;
			writeScore($cSubject,"MLight",3,$dir,"FOR P: M on red");
	...
	switch ($signal)
	{
		case "GREEN":
			$score = $score + 3;
			writeScore($cSubject,"CSignal",3,$dir,"FOR P: C on WALK");
...
//RULE 10M ------Motorist signal behavior -------------------
...
//11 M4	MOTORIST enters intersection
//----------------------------------------------------
function rule10M($seqs,$dir)
{	
	echo "<br>RULE 10M: Motorist behavior:red, slowed, green, entered<br>";
	global $buffer;	
	global $cSubject;
	...
	{	
		writeScore($cSubject,"10M",1,$dir,"");
		echo "(motorist) red - slowed down - green - entered intersection";
		return 1;
	}
	else 
	{echo "!! NO POINTS !!"; 
		writeScore($cSubject,"10M",0,$dir,"");
		return 0;}
}
